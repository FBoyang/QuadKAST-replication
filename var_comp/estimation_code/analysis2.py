...
print(f'Original Start and End: {Start}; {End}')
for extend in [0, 5000, 10000, 50000, 100000]:
    pStart_upstream=int(pStart)-extend
    pEnd_downstream=int(pEnd)+extend

    ## New startiing index
    Indices_bool = (bimfile.chr==CHR)&(bimfile.pos>=pStart_upstream)&(bimfile.pos<=pEnd_downstream)
    Start = np.where(Indices_bool==True)[0][0]
    End = np.where(Indices_bool==True)[0][-1]
    print(Start, End)
    print(f'######## {trait}:{gene}; Up-down stream: {extend}; Actual genotype # SNPs: {End-Start+1} ########')
    ## prepare covariates
    ...
    with open(f'{savePath}_{trait}_{gene}_stream_{extend}.pkl', 'wb') as handle:
    ...
    with open(f'{savePath}{trait}_{gene}_stream_{extend}.inter.pkl', 'wb') as handle:
        pickle.dump(results, handle, protocol=pickle.HIGHEST_PROTOCOL)
